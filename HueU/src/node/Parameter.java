...
public class Parameter extends Pane{
    ...
    public Parameter(String name , ParameterType pType) {
        ...
        else { isActiveNeeded = FALSE;}
    }

    public Parameter myClone(){
        return new Parameter(name,pType);
    }


    public Control getActive() { return active; }
    public Control getExtraActive() {return extraActive;}

    public void layoutsChanged(){
        myCircle.layoutsChanged();
//        active.setLayoutX(this.getParameterX());
        if (isActiveNeeded) {
            if (pType == ParameterType.IMAGE_INPUT) active.setLayoutX(this.getParameterX()+160-40.50390625);//40.50390625=active.getWidth()
    ...
    void setParameterY(double parameterY) { this.parameterY = parameterY; }
